...
if [ ! -f model.safetensors ]; then
  ...
  curl -L -o model.safetensors \
    "https://huggingface.co/<user>/<model-repo>/resolve/main/model.safetensors"
fi

# Ensure tokenizer/config files exist (these live in repo too, but harmless to (re)download)
[ -f config.json ] || curl -L -o config.json \
  "https://huggingface.co/<user>/<model-repo>/resolve/main/config.json"
[ -f tokenizer_config.json ] || curl -L -o tokenizer_config.json \
  "https://huggingface.co/<user>/<model-repo>/resolve/main/tokenizer_config.json"
[ -f special_tokens_map.json ] || curl -L -o special_tokens_map.json \
  "https://huggingface.co/<user>/<model-repo>/resolve/main/special_tokens_map.json"
[ -f vocab.txt ] || curl -L -o vocab.txt \
  "https://huggingface.co/<user>/<model-repo>/resolve/main/vocab.txt"